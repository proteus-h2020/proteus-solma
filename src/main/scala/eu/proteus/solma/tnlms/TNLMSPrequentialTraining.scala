...
import eu.proteus.solma.tnlms.TNLMS.TNLMSStreamEvent
import eu.proteus.solma.tnlms.algorithm.TNLMSAlgorithm
...
@Proteus
class TNLMSPrequentialTraining extends TransformDataStreamOperation[
      TNLMS,
      TNLMS.TNLMSStreamEvent,
      Either[(TNLMS.UnlabeledVector, Double), (Int, TNLMS.TNLMSModel)]] {


  def init(a: Double, b: Double, n: Int): Int => TNLMS.TNLMSModel =
    _ => (DenseVector.fill(n){a}, b)
  ...
  override def transformDataStream(
      ...
      input: DataStream[TNLMSStreamEvent]
  ): DataStream[Either[(TNLMS.UnlabeledVector, Double), (Int, TNLMS.TNLMSModel)]] = {
    ...
    val updater = (currModel: TNLMS.TNLMSModel, gradient: TNLMS.TNLMSModel) => {
    ...
    val workerLogic =  WorkerLogic.addPullLimiter(
      new TNLMSWorkerLogic(
        new TNLMSAlgorithm(instance)),
      pullLimit)

    val serverLogic = new RangePSLogicWithClose[TNLMS.TNLMSModel](featureCount, initializer, updater)
    val paramPartitioner: WorkerToPS[TNLMS.TNLMSModel] => Int = rangePartitionerPS(featureCount)(psParallelism)

    val wInPartition: PSToWorker[TNLMS.TNLMSModel] => Int = {
      case PSToWorker(workerPartitionIndex, _) => workerPartitionIndex
    }

    implicit val inputTypeInfo = createTypeInformation[TNLMSStreamEvent]
