...
@Proteus
class FrequentDirectionsITSuite
    extends FlatSpec
    with Matchers
    with FlinkTestBase {

  import FrequentDirections._
  import FrequentDirectionsITSuite._

  behavior of "Flink's Frequent Directions"

  it should "perform frequent directions without final merge" in {
    ...
    val stream = env.fromCollection(FrequentDirectionsITSuite.data)
  ...
  it should "perform frequent directions with final merge" in {
    val env = StreamExecutionEnvironment.getExecutionEnvironment
...
object FrequentDirectionsITSuite {
