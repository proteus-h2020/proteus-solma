...
import eu.proteus.solma.pa.PA.UnlabeledVector
...
class PAITSuite extends FlatSpec
  with Matchers
  ...
  it should "perform binary PA on plain Nursey data" in {
    helper(dataNotShuffled)
  }

  it should "perform binary PA on semi shuffled Nursey data" in {
...
object PAITSuite {
  ...
  val LOG: slf4j.Logger = LoggerFactory.getLogger(PAITSuite.getClass)
  ...
  def helper(data: Seq[Either[(Long, StreamEvent), (Long, Double)]]): Unit = {
    ...
    val predictionsAndModel = pa.transform(ds)
    ...
    predictionsAndModel
      ...
      .flatMap(coFlatMapper = new RichCoFlatMapFunction[Either[(Long, UnlabeledVector, Double), (Int, linalg.DenseVector[Double])], (Long, Double), BinaryConfusionMatrix] {
