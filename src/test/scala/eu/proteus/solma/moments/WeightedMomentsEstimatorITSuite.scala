...
import eu.proteus.solma.events.StreamEvent2
...
case class WeightedSample (
                    s: IndexedSeq[Int],
                    w:DenseVector,
                    d: DenseVector) extends StreamEvent2 {
  override val slice: IndexedSeq[Int] = s
  override val weight:Vector=w
  override val data: Vector = d
}
case class weightedMeanAndVariance( samples:Seq[WeightedSample],featurecount:Int) //, v:DenseVector)
{ private val meaa=samples.fold(WeightedSample(IndexedSeq(),DenseVector.zeros(featurecount),DenseVector.zeros(featurecount)))((i,j)=> {
  val temp=i.data.asBreeze.copy
  ...
  private val varaa=samples.fold(WeightedSample(IndexedSeq(),DenseVector.zeros(featurecount),DenseVector.zeros(featurecount)))((i,j)=> {
    val temp=i.data.asBreeze.copy
    temp(j.slice):+=j.weight.asBreeze.copy:*(j.data.asBreeze.copy:-weightedmean(j.slice)):*(j.data.asBreeze.copy-weightedmean(j.slice))
