...
class COIRRITSuite extends FlatSpec
    with Matchers
    with FlinkTestBase {

  behavior of "SOLMA Online SLOG"

  import COIRRITSuite.Sample

  it should "perform binary COIRR" in {
    ...
    coirr.setFeaturesCount(3)
    coirr.setPullLimit(10000)
    coirr.setIterationWaitTime(20000)

    val ds: DataStream[Either[(Long, StreamEvent), (Long, Double)]] = env.fromCollection(COIRRITSuite.data)

    coirr.transform(ds).print()
