...
import eu.proteus.solma.coirr.COIRR.{COIRRStreamEvent, COIRRModel, UnlabeledVector}
import eu.proteus.solma.coirr.algorithm.COIRRParameterInitializer.initConcrete


object COIRRAlgorithmTest {
  val testsFilePath = "/fried_delve.txt"
  val initA = 0.0 // initialization of a value (COIRR model)
  val initB = 0.0 // initialization of b value (COIRR model)
  val initC = 0.1 // initialization of c value (COIRR model)
  val featureCount = 10
...
class COIRRAlgorithmTest extends FlatSpec with PropertyChecks with Matchers {
  import COIRRAlgorithmTest._

  "COIRR Algorithm" should "give reasonable error on test data" in {
    val TOTAL_SAMPLES = Source.fromURL(getClass.getResource(testsFilePath)).getLines.size
    val initLambda = 0.0005
    var processedSamples = 0
    var instance = new COIRRAlgorithm(new COIRR())
