...
class AARITSuite extends FlatSpec
    with Matchers
    with FlinkTestBase {

  behavior of "SOLMA Online AAR"

  import AARITSuite.Sample

  it should "perform binary AAR" in {
    ...
    aar.setFeaturesCount(3)
    aar.setPullLimit(10000)
    aar.setIterationWaitTime(20000)

    val ds: DataStream[Either[(Long, StreamEvent), (Long, Double)]] = env.fromCollection(AARITSuite.data)

    aar.transform(ds).print()
