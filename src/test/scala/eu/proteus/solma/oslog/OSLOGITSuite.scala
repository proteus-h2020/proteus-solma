...
class OSLOGITSuite extends FlatSpec
    with Matchers
    with FlinkTestBase {

  behavior of "SOLMA Online SLOG"

  import OSLOGITSuite.Sample

  it should "perform binary OSLOG" in {
    ...
    oslog.setFeaturesCount(3)
    oslog.setPullLimit(10000)
    oslog.setIterationWaitTime(20000)

    val ds: DataStream[Either[(Long, StreamEvent), (Long, Double)]] = env.fromCollection(OSLOGITSuite.data)

    oslog.transform(ds).print()
