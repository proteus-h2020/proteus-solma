...
class TNLMSITSuite extends FlatSpec
    with Matchers
    with FlinkTestBase {

  behavior of "SOLMA Online SLOG"

  import TNLMSITSuite.Sample

  it should "perform binary TNLMS" in {
    ...
    tnlms.setFeaturesCount(3)
    tnlms.setPullLimit(10000)
    tnlms.setIterationWaitTime(20000)

    val ds: DataStream[Either[(Long, StreamEvent), (Long, Double)]] = env.fromCollection(TNLMSITSuite.data)

    tnlms.transform(ds).print()
